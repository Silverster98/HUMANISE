...
def render_reconstructed_motion_in_scene(
    ...
    for i in range(S):
        ...
        material_gray = pyrender.MetallicRoughnessMaterial(
            metallicFactor=0.0,
            alphaMode='OPAQUE',
        ...
        scene = pyrender.Scene()
        if body_rec is not None:
            body_mesh_rec = pyrender.Mesh.from_trimesh(body_rec, material=material_gray)
            scene.add(body_mesh_rec)
        if body_gt is not None:
            body_mesh_gt = pyrender.Mesh.from_trimesh(body_gt, material=material_blue)
