...
from model.pointtransformer import pointops
...
class PointTransformerFea(nn.Module):
    def __init__(self, block, blocks, c=6):
        super().__init__()
        self.c = c
        self.in_planes, planes = c, [32, 64, 128, 256, 512]
        fpn_planes, fpnhead_planes, share_planes = 128, 64, 8
    ...
    def forward(self, pxo):
        ...
        pxos = []
        pxos.append([p1, x1, o1])
        pxos.append([p2, x2, o2])
        pxos.append([p3, x3, o3])
        pxos.append([p4, x4, o4])
        pxos.append([p5, x5, o5])
        return pxos

def pointtransformer_feature_extractor(**kwargs):
    model = PointTransformerFea(PointTransformerBlock, [2, 3, 4, 6, 3], **kwargs)
    return model

class PointTransformerDec(nn.Module):
    def __init__(self, block, blocks, c=6, k=2):
        super().__init__()
        self.c = c
        self.in_planes, planes = 512, [32, 64, 128, 256, 512]
        fpn_planes, fpnhead_planes, share_planes = 128, 64, 8
        stride, nsample = [1, 4, 4, 4, 4], [8, 16, 16, 16, 16]
...
def pointtransfromer_target_regressor(**kwargs):
    model = PointTransformerDec(PointTransformerBlock, [2, 3, 4, 6, 3], **kwargs)
    return model